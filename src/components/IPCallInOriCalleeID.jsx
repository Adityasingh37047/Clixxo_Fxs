  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editIndex !== null) {
        ...
        response = await updateNumberManipulation(updateData, 'ip_in_oricalleeid');
        ...
        response = await createNumberManipulation(normalized, 'ip_in_oricalleeid');
    ...
    <div className="bg-gray-50 min-h-[calc(100vh-200px)] flex flex-col items-center box-border" style={{backgroundColor: "#dde0e4"}}>
      {loading.fetch ? (
        ...
        <div className="w-full h-full flex flex-col items-center justify-center" style={{ minHeight: '15vh' }}>
          <div className="text-gray-600 text-xl md:text-[16px] font-semibold mb-4 text-center">No available number manipulation rule (IP Call In OriCalleeID)!</div>
          <Button
            variant="contained"
            sx={{
              ...blueButtonSx,
            ...
            <div className="bg-white rounded-lg shadow-sm w-full flex flex-col overflow-hidden">
              ...
              >
                <table className="w-full min-w-[1400px] border border-gray-300 border-collapse whitespace-nowrap" style={{ tableLayout: 'auto', border: '1px solid #bbb' }}>
                  <thead>
                    <tr style={{ minHeight: 32 }}>{IP_CALL_IN_ORICALLEEID_TABLE_COLUMNS.map(c => <th key={c.key} className="bg-white text-[#222] font-semibold text-[12px] border border-gray-300 text-center" style={{ border: '1px solid #bbb', padding: '6px 8px', minHeight: 32, whiteSpace: 'nowrap' }}>{c.label}</th>)}</tr>
                  </thead>
                  <tbody>
      ...
      >
        ...
        >
          {formData.originalIndex !== undefined ? 'Edit IP Call In OriCalleeID' : 'Add IP Call In OriCalleeID'}
        </DialogTitle>
        <DialogContent 
