...
# Routes are loaded on system boot via /etc/network/if-up.d/load-ip-routes
...
`;
      ...
      // Create/update startup script if it doesn't exist
      const scriptContent = `#!/bin/bash
# Auto-generated script to load IP routes on boot
# This script is executed when network interfaces come up

ROUTES_FILE="${ROUTES_CONFIG_FILE}"

if [ -f "$ROUTES_FILE" ]; then
    # Wait for interfaces to be up (especially VPN interfaces)
    sleep 2
    
    # Execute each route command from the config file
...
`;
      const createScriptCmd = `cat > ${ROUTES_SCRIPT_FILE} << 'EOF'\n${scriptContent}EOF\nchmod 755 ${ROUTES_SCRIPT_FILE}`;
      await postLinuxCmd({ cmd: createScriptCmd });
  ...
  useEffect(() => {
    ...
    if (Array.isArray(persisted) && persisted.length) setRows(persisted);
  }, []);
  ...
  const handleSave = async (e) => {
    ...
    postLinuxCmd({ cmd: deleteCmd })
      ...
      .then(() => {
        // Persist to UI table (include gateway in saved row)
        let updatedRows;
        if (editIndex !== null) {
          updatedRows = [...rows];
          updatedRows[editIndex] = { ...form, checked: false, gateway: gateway || form.gateway || '' };
          setRows(updatedRows);
        } else {
          updatedRows = [...rows, { ...form, checked: false, no: rows.length, gateway: gateway || form.gateway || '' }];
          setRows(updatedRows);
        }
        ...
        saveRoutesToFile(updatedRows, networkOptions).then(saved => {
          if (saved) {
            alert('Route added successfully and saved to persistent configuration!');
          } else {
            alert('Route added successfully, but failed to save to persistent configuration file. Route will be lost on reboot.');
          }
        });
  ...
  const handleDelete = () => {
    ...
    Promise.allSettled(deletePromises).then(async () => {
      const remainingRows = rows.filter((_, idx) => !itemsToDelete.some(i => i.idx === idx));
      setRows(remainingRows);
      
      // Update persistent config file with remaining routes
      await saveRoutesToFile(remainingRows, networkOptions);
      alert(`${itemsToDelete.length} route(s) deleted successfully and persistent configuration updated!`);
    });
  };
  ...
  const handleClearAll = () => {
    ...
    Promise.allSettled(deletePromises).then(async () => {
      ...
      await saveRoutesToFile([], networkOptions);
      alert('All routes deleted successfully and persistent configuration cleared!');
    });
  };
    ...
    <div className="bg-gray-50 min-h-[calc(100vh-200px)] p-2 md:p-0">
      <div className="w-full max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center bg-gray-300 rounded-b-lg px-1 py-0.5 mt-1 border-l-2 border-r-2 border-b-2 border-gray-400">
          ...
          <button className="bg-gray-400 text-gray-700 font-semibold text-xs rounded px-2 py-0.5 min-w-[70px] shadow hover:bg-gray-500" onClick={() => openModal(null)}>Add New</button>
        </div>
      </div>
      {/* Modal */}
