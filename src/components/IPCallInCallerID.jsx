  ...
  const fetchNumberManipulations = async () => {
    ...
    try {
      ...
      if (error.message === 'Network Error') {
        ...
        showMessage('error', 'Server error. The number manipulations endpoint may have issues.');
      } else {
        showMessage('error', error.message || 'Failed to load number manipulations');
      }
      setRules([]);
    } finally {
  ...
  return (
    <div className="bg-gray-50 min-h-[calc(100vh-200px)] flex flex-col items-center box-border" style={{backgroundColor: "#dde0e4"}}>
              ...
              <div className="w-full border-b border-gray-300" style={{ borderBottomLeftRadius: 0, borderBottomRightRadius: 0, borderBottom: 'none' }}>
                ...
                >
                  <table className="w-full min-w-[1400px] border border-gray-300 border-collapse whitespace-nowrap" style={{ tableLayout: 'auto', border: '1px solid #bbb' }}>
                    <thead>
                      <tr style={{ minHeight: 32 }}>{IP_CALL_IN_CALLERID_TABLE_COLUMNS.map(c => <th key={c.key} className="bg-white text-[#222] font-semibold text-[12px] border border-gray-300 text-center" style={{ border: '1px solid #bbb', padding: '6px 8px', minHeight: 32, whiteSpace: 'nowrap' }}>{c.label}</th>)}</tr>
                    </thead>
                    <tbody>
      ...
      >
        ...
        >
          {formData.originalIndex !== undefined ? 'Edit IP Call In CallerID' : 'Add IP Call In CallerID'}
        </DialogTitle>
        <DialogContent 
