  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editIndex !== null) {
        ...
        response = await updateNumberManipulation(updateData, 'pstn_in_calleeid');
        ...
        response = await createNumberManipulation(normalized, 'pstn_in_calleeid');
  ...
  return (
    <div className="bg-gray-50 min-h-[calc(100vh-200px)] flex flex-col items-center box-border" style={{backgroundColor: "#dde0e4"}}>
            ...
            <div className="bg-white w-full flex flex-col overflow-hidden" style={{ borderTopLeftRadius: 0, borderTopRightRadius: 0 }}>
              ...
              >
                <table className="w-full min-w-[1400px] border border-gray-300 border-collapse whitespace-nowrap" style={{ tableLayout: 'auto', border: '1px solid #bbb' }}>
                  <thead>
                    <tr style={{ minHeight: 32 }}>{PSTN_CALL_IN_CALLEEID_TABLE_COLUMNS.map(c => <th key={c.key} className="bg-white text-[#222] font-semibold text-[12px] border border-gray-300 text-center" style={{ border: '1px solid #bbb', padding: '6px 8px', minHeight: 32, whiteSpace: 'nowrap' }}>{c.label}</th>)}</tr>
                  </thead>
                  <tbody>
      ...
      >
        ...
        >
          {editIndex !== null ? 'Edit PSTN Call In CalleeID' : 'Add PSTN Call In CalleeID'}
        </DialogTitle>
        <DialogContent 
